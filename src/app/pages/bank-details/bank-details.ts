...
import { timer, switchMap, take, takeWhile } from 'rxjs';
...
export class BankDetails implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // The route id is the transaction id. Keep txId as a backwards-compatible
    // query-param override for links generated by older versions of the FE.
  ...
  private restoreVerificationState() {
    ...
    this.txService.getById(txId).subscribe({
      next: tx => {
  ...
  private launchSmileIdSdk(session: SmileSession) {
    ...
    SmileIdentity({
      ...
      onSuccess: (_data: unknown) => {
        // SmileID success means the widget completed. The backend remains the
        // source of truth for final ID + liveness approval.
      ...
      onClose: () => {
        // Do not reset a successful/polling verification back to the bank form.
        // The SDK can close after its success callback while backend processing
        // is still completing.
  ...
  private pollSellerVerification() {
    ...
    timer(0, 3000).pipe(
      ...
      complete: () => {
        if (!terminal && this.kycState() === 'pending') {
